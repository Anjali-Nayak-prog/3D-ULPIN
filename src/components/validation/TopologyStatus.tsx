...
export function TopologyStatus() {
  return (
    <div className="rounded-xl border border-slate-200 bg-navy-900/70 p-5">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-slate-900">Topology Status</h3>
          <p className="mt-0.5 text-xs text-slate-500">Live integrity of the volumetric graph</p>
        </div>
        <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-[11px] font-medium text-emerald-600">
          Overall 98.8%
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        {checks.map((check) => (
          <div key={check.name} className="rounded-lg border border-slate-200 bg-slate-100/50 px-3.5 py-3">
